...
节点使用 `ops` SSH 用户，公钥由 Cloud-init 写入，Ansible 通过 sudo 执行系统任务。
...
1. 通过 `ops` 用户配置 Debian 节点的 QEMU Guest Agent、NTP 时间同步、内核模块、sysctl 与基础软件。
2. 在 `cd-k3s-server-01` 安装并启动 K3s。
3. 部署 kube-vip，以 ARP 模式提供 API VIP `10.10.10.69:6443`。
4. 为 Traefik 提供局域网 Ingress VIP `10.10.10.68:443`。
